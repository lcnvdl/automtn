...
(function ($) {
	 ...
	 *	@param {String}		[options.parsemode="none"] Post-parses the content. Values: `none`, `kv-as-values`, `kv-as-array`, `array-as-kv`.
            ...
            if (action == 'fill') {
                ...
                var ajaxOpts = {
                    ...
                    success: function (result) {
                        var response = { ajax: this, success: typeof result.success === 'undefined' ? true : result.success, result: result };

                        if (!response.success && typeof result.error !== 'undefined') {
                            response.error = result.error;
                        }

                        try {
                            this.trigger('beforefill', response);
                            Automtn.call(this, beforefill, response);

                            this.automtn({ action: 'fill', data: result });

                            this.trigger('onloadsuccess', response);
                            Automtn.call(this, loadsuccess, response);
                        }
                        catch (ex) {
                            var errorResponse = $.extend(response, { success: false, error: ex });

                            this.trigger('onloaderror', errorResponse);
                            Automtn.call(this, loaderror, errorResponse);
                        }
                        finally {

                            Automtn.call(this, finishload, response);
                            this.trigger('onloadfinish', response);
                        }
                    },
                    error: function (e, algo, thrownError) {
                        var msg;
                        ...
                        var errorResponse = { success: false, error: msg };

                        this.trigger('onloaderror', errorResponse);
                        this.trigger('onloadfinish', errorResponse);
...
var Automtn = {
    ...
    each: function (e, data, fn) {
        ...
        else if (parsemode == "array-as-kv") {
            var i = 0;
            var nresul = {};

            $.each(resul, function (key, value) {
                nresul[value[0]] = value[1];
            });
